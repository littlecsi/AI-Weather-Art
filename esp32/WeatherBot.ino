...
const char* ssid = "Leo";         // YOUR WIFI SSID / NAME
const char* password = "leo2002!";   // YOUR WIFI PASSWORD


const int servoSpeed = 16;                // Controls the speed of rotation of the servos (0 to 90 with 90 being the fastest) Default = 23

String serverPath = "http://api.openweathermap.org/data/3.0/onecall?lat=51.501949&lon=-0.159640&units=metric&exclude=minutely&appid=667bd65d9a4510922415641d4b384295";  // Your API call made up as follows: http://api.openweathermap.org/data/2.5/onecall?lat={latitude}}&lon=-{longitude}}&units=metric&exclude=minutely&appid={Your-API-Key}
...
void loop() {
  ...
  if ((millis() - lastTime) > timerDelay) {
    ...
    if (WiFi.status() == WL_CONNECTED) {
      ...
      weatherTodaysDescription = JSON.stringify(myObject["daily"][0]["weather"][0]["description"]);
      ...
      weatherTomorrowsDescription = JSON.stringify(myObject["daily"][1]["weather"][0]["description"]);
